...
const { BLOGS_PATH, ITEMS_PER_PAGE } = require("../config");
const { faker } = require("@faker-js/faker");

const PAGE_URI = "/" + BLOGS_PATH;
const BLOG_URI = "/" + BLOGS_PATH;
    ...
    request(app)
      ...
      .end((err, res) => {
        ...
        try {
          ...
          validatePage(response, pageSchema, blogItemSchema);
  ...
  return (done) => {
    request(app)
      ...
      .end((err, res) => {
        ...
        try {
          const response = JSON.parse(res.text);
          validateSchema(response, blogSchema);

          validateValue(response, "uid", uid);

          validateValue(response, "_links.self.href", BLOG_URI + "/" + uid);
          validateValue(response, "_links.self.title", response.title);

          validateValue(response, "_links.update.href", BLOG_URI + "/" + uid);
          validateValue(response, "_links.update.title", "Gem");
          validateValue(response, "_links.update.method", "PATCH");

          validateValue(response, "_links.delete.href", BLOG_URI + "/" + uid);
    ...
    request(app)
      ...
      .end((err, res) => {
        ...
        try {
          ...
          //validateValue(response, "image", data.image);
          validateValue(response, "content", data.content);

          validateValue(
            response,
            "_links.self.href",
            BLOG_URI + "/" + response.uid
          );
...
describe("GET " + PAGE_URI, () => {
  ...
  it(
    "response with page 2 and 100 blogs",
...
describe("GET " + PAGE_URI + "/:uid", () => {
  it("response with a blog", testUnit(PAGE_URI, 0, 0));
  it("response in case of blog not found", testError(PAGE_URI + "/" + 500));
});

describe("GET " + PAGE_URI + "/:slug", () => {
  it("response with a blog", testUnit(PAGE_URI, "custom-blog-title", 30));
  it(
    "response in case of blog not found",
    testError(PAGE_URI + "/custom-blog-title2")
  );
});

describe("POST " + PAGE_URI, () => {
  it("response with a blog", testPost(PAGE_URI));
  it(
